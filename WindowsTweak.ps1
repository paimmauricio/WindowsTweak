...
if (-not ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) {
    Write-Host "Este script requer privilégios de administrador. Execute-o novamente como Administrador." -ForegroundColor Red
    Exit
}

# Caminho do backup do Registro
$backupPath = "C:\Backup_Tweak.reg"

# Pergunta ao usuário se deseja restaurar o backup
$restore = Read-Host "Deseja restaurar o backup anterior? (S/N)"
if ($restore -match "^[sS]$") {
    if (Test-Path $backupPath) {
        Write-Host "Restaurando configurações do backup..." -ForegroundColor Yellow
        Start-Process "reg" -ArgumentList "import $backupPath" -NoNewWindow -Wait
        Write-Host "Configurações restauradas. Reinicie o computador para aplicar as mudanças." -ForegroundColor Green
        Exit
    } else {
        Write-Host "Nenhum backup encontrado. Continuando com as otimizações..." -ForegroundColor Red
    }
}

# Criando backup antes de modificar o Registro
Write-Host "Criando backup das configurações atuais..." -ForegroundColor Yellow
reg export "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects" $backupPath /y
...
$services = @(
    ...
    "DiagTrack", "dmwappushservice",      # Telemetria e diagnósticos
...
# 10. Reduz processos em segundo plano
...
wsreset # Limpa cache Microssot Store
Dism.exe /online /Cleanup-Image /StartComponentCleanup /ResetBase   # Remove pacotes antigos do Windows Update-AutologgerConfig

# 12. Verificação e reparo
Write-Host "Verificando e reparando integridade do sistema...Se aparecer PRED FAIL disco pode estar com defeito"
sfc /scannow
DISM /Online /Cleanup-Image /RestoreHealth
wmic diskdrive get model,status   # Verifica temperatura do disco
